...
impl Chip8 {
    ...
    pub fn step(&mut self, video_engine: &mut VideoEngine, debugger: &mut Debugger) {
        ...
        match Instruction::try_from(opcode) {
            ...
            Ok(instruction) => self.step_instruction(instruction, video_engine, debugger),
        }
    }
    ...
    fn step_instruction(&mut self, instruction: Instruction, video_engine: &mut VideoEngine, debugger: &mut Debugger) {
        println!("0x{0:03x} {1:?}", self.pc, instruction);
        debugger.current_location(&self.pc);
        match instruction {
